...
const loginScreen = document.getElementById('login-screen'); // <-- NOVA REFERÊNCIA
...
const loginButtons = document.querySelectorAll('#login-screen button'); // <-- NOVA REFERÊNCIA para botões de login

function switchScreen(screenToShow) {
    appContainer.classList.add('opacity-0');
    setTimeout(() => {
        loginScreen.classList.add('hidden'); // <-- Adicionado para ocultar a tela de Login
...
// NOVO: Event listeners para os botões da tela de login
loginButtons.forEach(button => {
    button.addEventListener('click', (e) => {
        const loginType = button.dataset.loginType;
        
        // Simula o login e avança para a seleção de perfil
        console.log(`Tentativa de Login com: ${loginType}`);
...
document.addEventListener('DOMContentLoaded', () => {
    switchScreen(loginScreen); // <-- Começa na tela de Login
});
